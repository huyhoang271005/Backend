...
public class VerifyService {
    ...
    private void checkResendRateLimit(UUID typeId, VerificationTypes type) {
        ...
        if (!verificationToken.isEmpty()) {
            //Nếu còn hạn sử dụng
            if (verificationToken.getLast().getExpiredAt().isAfter(Instant.now())) {
    ...
    public Response<Void> sendVerifyEmail(EmailRequest emailRequest) {
        ...
        emailVerifyService.sendEmail(emailRequest.getEmail(), StringApplication.FIELD.VERIFY + StringApplication.FIELD.EMAIL, userEmail.getUser().getProfile().getFullName(), CorsConfig.BASE_URL + "/WebProject/auth/verified?verificationId=" + verificationToken.getVerificationTokenId(), timeExpired + StringApplication.FIELD.HOURS);
        return new Response<>(true, StringApplication.SUCCESS.CHECK_EMAIL, null);
    }


    @Transactional
    public Response<DeviceResponse> sendVerifyDevice(EmailRequest emailRequest, UUID deviceId, String userAgent, String deviceType, String deviceName, String ip) {
        //Kiểm tra email
        var userEmail = emailRepository.findByEmail(emailRequest.getEmail()).orElseThrow(() -> new EntityNotFoundException(StringApplication.FIELD.EMAIL + StringApplication.FIELD.NOT_EXIST));
        ...
        var device = Optional.ofNullable(deviceId).flatMap(deviceRepository::findById).orElseGet(() -> Device.builder().deviceName(deviceName).deviceType(deviceType).userAgent(userAgent).build());
        deviceRepository.save(device);
        //Lấy dữ liệu user và session(nếu không có thì tạo mới session)
        var user = userEmail.getUser();
        var session = sessionRepository.findByUserAndDevice(user, device).orElseGet(() -> Session.builder().device(device).user(user).ipAddress(ip).revoked(false).validated(false).build());
        ...
        var verificationToken = VerificationTokens.builder().expiredAt(Instant.now().plusSeconds(timeExpired)).verificationType(VerificationTypes.VERIFY_DEVICE).typeId(session.getSessionId()).user(user).build();
        verificationTokensRepository.save(verificationToken);
        //Gửi email xác thực tới user
        emailVerifyService.sendEmail(emailRequest.getEmail(), StringApplication.FIELD.VERIFY + StringApplication.FIELD.DEVICE, userEmail.getUser().getProfile().getFullName(), CorsConfig.BASE_URL + "/WebProject/auth/verified?verificationId=" + verificationToken.getVerificationTokenId(), timeExpired + StringApplication.FIELD.HOURS);
    ...
    public Response<Void> verify(String token) {
        ...
        if (tokenVerify.getVerificationType() == VerificationTypes.VERIFY_EMAIL) {
            //Tìm email
            var userEmail = emailRepository.findById(tokenVerify.getTypeId()).orElseThrow(() -> new ConflictException(StringApplication.FIELD.EMAIL + StringApplication.FIELD.INVALID));
        ...
        //Nếu là xác thực session
            ...
            //Tìm session
            var session = sessionRepository.findById(tokenVerify.getTypeId()).orElseThrow(() -> new EntityNotFoundException(StringApplication.FIELD.SESSION_LOGIN + StringApplication.FIELD.INVALID));
    ...
    public Response<Void> sendVerifyChangePassword(EmailRequest emailRequest) {
        ...
        emailVerifyService.sendEmail(userEmail.getEmail(), StringApplication.FIELD.VERIFY + StringApplication.FIELD.CHANGE_PASSWORD, user.getProfile().getFullName(), CorsConfig.BASE_URL + "/WebProject/auth/change-password?token=" + verifyToken.getVerificationTokenId(), minExpired + StringApplication.FIELD.MINUTES);
